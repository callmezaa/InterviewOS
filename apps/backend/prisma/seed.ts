import { PrismaClient, Role, InterviewStatus, QuestionDifficulty, QuestionSource, QuestionStatus } from '@prisma/client';
...
async function main() {
  ...
  await prisma.interview.upsert({
    ...
    create: {
      ...
      participants: {
        create: [
          { userId: interviewer.id, role: Role.INTERVIEWER },
  ...
  const authorUser = await prisma.user.findFirst({ where: { role: Role.INTERVIEWER } });
