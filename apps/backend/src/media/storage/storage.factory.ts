 ...
 *
 * If the provider's SDK is not installed (or config invalid), the factory
 * falls back to local storage so the app still boots.
 */
  ...
  switch (provider) {
    case 's3': {
      try {
        ...
        logger.warn(
          `S3 storage unavailable (${(err as Error).message}). Falling back to local storage.`,
        );
    ...
    case 'gcs': {
      try {
        ...
        logger.warn(
          `GCS storage unavailable (${(err as Error).message}). Falling back to local storage.`,
        );
        return createLocalAdapter(config);
      }
    }
...
function createLocalAdapter(config: ConfigService): IStorageAdapter {
  const baseUrl =
